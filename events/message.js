...
module.exports = async (client, message) => {
  ...
  if (!timestamps.has(member.id)) {
    ...
    if (now < expirationTime) {
      ...
      return message.channel.send(`Sakin ol dostum, komutu tekrar denemek için lütfen **${timeLeft.toFixed(1)}** saniye beklemen gerek.`);
    }
  ...
  try {
    ...
    console.log(`${sender.tag} (${sender.id}) Şu Komutu Çalıştırdı: ${cmd}`);
  }
}